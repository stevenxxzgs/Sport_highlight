...
class Uploader(Thread):
    ...
    def upload_video(self, task):
        ...
        #         path.split('/')[-1]
        # print(type)
        success, video_info = self.upload_video_to_cloud(type, 'highlight/'+ court_num + '/' + folder_name_datatime_str + '/' + video_name)
    ...
    def upload_picture(self, task):
        type, path = task
        success, picture_info = self.upload_picture_to_cloud(type, 'highlight/'+ court_num + '/' + folder_name_datatime_str + '/' + picture_name)
        if not success: 
            print("Unsuccessful request. url: ", path)
...
video_path = os.path.join(current_path, 'output/clip_video/')
